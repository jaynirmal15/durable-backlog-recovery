# Paper 2 — section-by-section outline, v10.3

*2026-09-20. **v10.3 adds the B.E./B.S. degree check to the W6 hard pre-submission checks, beside "publicly archived".** **v10.2 records the two F4 figure decisions (y-axis (1700, 2105) not compressed; measured bars not re-centred) as FINAL — see the F4 row — and marks the W5 artefact renames DONE at `f44d2f1`.**
...
| **AI disclosure** | **Parked by Jay's decision: raise in W6, immediately before submission — not earlier.** Note the timing consequence: by W6 the paper is drafted, so the author-drafts-it route is already closed and the live choice is a disclosure line or nothing. Tooling (harness, analysis scripts, figure generators) needs no disclosure — instruments are not article content. Article *text* is covered by IEEE policy unless the author drafts it from the outline and AI only reviews and edits, which the policy exempts. The two workable paths: author drafts from the outline with AI review only (no disclosure, ~3 weeks slower), or AI drafts and one sentence goes in the Acknowledgments naming system, sections and level of use (schedule as planned). Paraphrasing an AI draft is not a third path — it still requires disclosure. |
...
| W6 Oct 20–26 | Final pass, AI disclosure resolved. **Re-stage the Zenodo package from the frozen commit, upload, verify, publish** — the reserved DOI must resolve before submission. Metadata to complete first: `description`, `related_identifiers` (repo URL, Paper 1's `10.5281/zenodo.22061184`), licence note covering `scripts/`, title naming the paper. **Hard pre-submission checks, each verified against a primary source and not against this outline:** (1) the abstract's "publicly archived" is true — the DOI resolves to a published record; (2) **the biography's first degree — B.E. or B.S. — matches Jay's University of Mumbai degree certificate** (the two CVs disagree: project CV "Bachelor of Engineering", iCloud `Jay_Nirmal_CV.pdf` "B.S."; the losing CV is corrected afterwards so the question cannot reopen). This row carries the check because the biography's editorial note is stripped before submission. Then submit. |
