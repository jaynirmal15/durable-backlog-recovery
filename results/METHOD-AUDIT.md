...
| 17. plateau repeatability | ~~**n = 1 per corrected cell.** No spread exists; none synthesised.~~ — **superseded 2026-09-20.** Addendum A8 (`590d1cc`) registered a replication before it ran and measured **ten 60-second windows per arm**: medians **1988.9641** and **1997.7031** rps, IQRs 1.71 and 1.29 rps (`results/A8-plateau-repeatability.json`, `results/A8-PLATEAU-REPEATABILITY.md`). The n = 1 limitation is **discharged and must not be restated** — §V, §IX and F4's caption all treat it as discharged, and the v4.6 instruction to caption F4 as n = 1 with no error bars is likewise obsolete. What still stands from the original entry: quote raw differences, "−0.00%" must not appear, and the seven cells have 6–15 each — the more load-bearing measurement is still the less replicated one |
...
### Artefact name versus manuscript number — RESOLVED at the W5 pass

This section recorded a name in the repository that did not mean what a
reader of the manuscript would take it to mean. **The W5 renames removed the
mismatch on 2026-09-20; file names and manuscript numbers now agree.** The
mapping is kept because commit messages, reports and plans written before that
date use the old names.

| repository | manuscript |
|---|---|
| `figures/T4-false-findings.md`, generated by `scripts/make_table4.py` | **Table 4** — findings that entered the record and were later killed |
| *(no generated artefact)* — lives in `paper/section4.md` | **Table 1** — the amendments, the A7 registered re-analysis, the A8 registered replication, and the two record corrections (A9, A10) |
| `figures/T2-resolution.md`, generated by `scripts/make_table2.py` (added 2026-09-19) | **Table 2** — per-cell resolution, under the maximum numerator. Here the file name and the manuscript number agree |
| `figures/T3-candidate-explanations.md`, generated by `scripts/make_table3.py` | **Table 3** — candidate explanations of the boundary and what calibration changed |

**Done at W5, 2026-09-20.** `T1-false-findings.md` → `T4-false-findings.md`
(`make_table1.py` → `make_table4.py`), `T3-resolution.md` →
`T2-resolution.md` (`make_table3.py` → `make_table2.py`), and
`calibration-artefact-findings.md` → `T3-candidate-explanations.md`
(`make_calibration_findings.py` → `make_table3.py`). The three artefacts
regenerate byte-identically apart from the generator name each prints.

**The third generator took the name the second vacated**, which is the one
ordering that can clobber a file. It was done last, after verifying the name
was free, and with `git mv`, which refuses to overwrite an existing path. The
alternative — giving it a name that collides with nothing — would have kept a
generator whose name did not match its table, which is the mismatch this
section exists to record.

**References written before the rename keep the old names where they are
pinned to a commit**, because at that commit the old name is what existed:
`paper/section8.md` citing `calibration-artefact-findings.md` at `c0a4eb2`,
and the outline at `37d3849`. Pointers to a current location were updated.
...
| 33. renames DONE | W5, 2026-09-20: names and manuscript numbers now agree — `T4-false-findings.md`, `T2-resolution.md`, `T3-candidate-explanations.md`, with generators `make_table4/2/3.py`. Commit-pinned references keep the old names |
...
`figures/T4-false-findings.md` — then named `T1-false-findings.md` — dropped
"which is impossible" on 2026-09-20.
