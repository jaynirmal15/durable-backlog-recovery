...
### Artefact name versus manuscript number: T1 is the manuscript's Table 2

Same purpose as the field-name table above: a name in the repository that does
not mean what a reader of the manuscript would take it to mean.

| repository | manuscript |
|---|---|
| `figures/T1-false-findings.md`, generated by `scripts/make_table1.py` | **Table 2** — findings that entered the record and were later killed |
...
| `figures/T3-resolution.md`, generated by `scripts/make_table3.py` (added 2026-09-19) | **Table 3** — per-cell resolution, under the maximum numerator. Here the file name and the manuscript number agree |

So "T1" in a file name, a commit message or this audit means the manuscript's
Table 2, not its Table 1.

**Flagged for renaming at the W5 figure pass, not now.** The artefact and its
generator keep their names until then, because renaming them touches the
manifest and every reference to them at once, and that belongs with the other
figure-pass changes.
